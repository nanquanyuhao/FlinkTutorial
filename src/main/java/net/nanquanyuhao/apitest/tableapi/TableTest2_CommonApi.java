package net.nanquanyuhao.apitest.tableapi;/**
...
public class TableTest2_CommonApi {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 2. 表的创建：连接外部系统，读取数据
        // 2.1 读取文件
        String filePath = "D:\\code\\job\\FlinkTutorial\\src\\main\\resources\\sensor.txt";
        ...
        Table aggTable = inputTable.groupBy("id")
                // 其他字段的 count 也可，或者 count(字段)及count(1)
                // Cannot resolve field [*], input field list:[id, timestamp, temp].
                .select("id, id.count as count, temp.avg as avgTemp");

        // 3.2 SQL
        tableEnv.sqlQuery("select id, temp from inputTable where id = 'senosr_6'");
        Table sqlAggTable = tableEnv
                .sqlQuery("select id, count(id) as cnt, avg(temp) as avgTemp from inputTable group by id");
